...
export async function POST(req: Request): Promise<Response> {
  ...
  const [{ data: historyRows }, { data: profile }] = await Promise.all([
    db
      .from("kori_messages")
      .select("role, content")
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const { data: assistantMessage, error } = await db
          .from("kori_messages")
          .insert({
        ...
        // Touch the conversation so history sorts by recency.
        await db
          .from("kori_conversations")
          .update({ model_used: profile?.preferred_model ?? null })
          .eq("id", conversationId)
        controller.enqueue(sseChunk("done", { assistantMessageId: assistantMessage.id }))
      } catch (err) {
        controller.enqueue(
